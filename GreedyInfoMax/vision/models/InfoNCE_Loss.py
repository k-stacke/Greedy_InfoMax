...
class InfoNCE_Loss(nn.Module):
    def __init__(self, opt, in_channels, out_channels):
        ...
        self.prediction_directions = ['top-down', 'buttom-up', 'left-right', 'right-left'][:opt.pred_directions]
        print('Prediction in directions:', self.prediction_directions)
    ...
    def forward(self, z, c, skip_step=1):
        ...
        for k in range(1, self.k_predictions + 1):
            for direction in self.prediction_directions:
                if direction == 'top-down':
                    # Predict top-down
                    z_sub = z[:, :, (k + skip_step):, :]  # Bx, C , H , W
                    c_sub = c[:, :, :-(k + skip_step), :]
                elif direction == 'buttom-up':
                    # Predict bottom-up
                    z_sub = z[:, :, :-(k + skip_step) , :]
                    c_sub = c[:, :, (k + skip_step):, :]
                ...
                #### Compute  x_W1 . c_t:
                context = (
                    c_sub.permute(2, 3, 0, 1).unsqueeze(-2)
                )  # y, x, b, 1, c
