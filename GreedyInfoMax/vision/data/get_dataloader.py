...
def get_camelyon_dataloader(opt):
    ...
    if opt.training_data_csv:
        ...
        if os.path.isfile(f"{opt.data_input_dir}/lnco_camelyon_patches.csv"):
            print("reading csv file")
            df = pd.read_csv(f"{opt.data_input_dir}/lnco_camelyon_patches.csv")
        else:
            raise Expetion(f"Cannot find file {opt.data_input_dir}/lnco_camelyon_patches.csv")
        ...
        training_size = int(len(request_ids)*0.7) # 70% training data
        validation_size = len(request_ids) - training_size
        train_req_ids.extend([request_id for request_id in request_ids[:training_size]])  # take first
        valid_req_ids.extend([
            request_id for request_id in request_ids[training_size:training_size+validation_size]])  # take last
    
        print("train / valid / total")
        print(f"{len(train_req_ids)} / {len(valid_req_ids)} / {len(request_ids)}")
    
        train_df = df[df.request_id.isin(train_req_ids)]
        val_df = df[df.request_id.isin(valid_req_ids)]
    ...
    train_df.to_csv(f'{opt.data_output_dir}/training_patches.csv', index=False)
    val_df.to_csv(f'{opt.data_output_dir}/test_patches.csv', index=False)
...
def clean_data(img_dir, dataframe):
    """ Clean the data """
    available_images = {file_ for file_ in os.listdir(f"{img_dir}/camelyon17_imagedata")}
    for row in dataframe.iterrows():
        if f"{row[0]}.jpg" not in available_images:
            print(f"Removing non-existing file from dataset: {row[0]}.jpg")
